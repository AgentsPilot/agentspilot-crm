      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Overdue', value: overdueCount, color: 'text-orange-600', bg: 'bg-orange-50' },
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            {DAY_FILTERS.map(f => (
              <button key={f} onClick={() => setDayFilter(f)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                  dayFilter === f ? 'bg-indigo-600 text-white' : 'bg-white border border-gray-200 text-slate-600 hover:bg-gray-50'
                }`}>
                {f}
            ...
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)}
              className="text-sm border border-gray-200 rounded-lg px-3 py-1.5 bg-white text-slate-700">
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              ...
              {pending.length === 0 ? (
                ...
                <div className="divide-y divide-gray-50">
                  {pending.map(task => (
                    <div key={task.id} className={`flex items-start gap-3 px-5 py-4 hover:bg-gray-50 transition-colors ${isOverdue(task.due_date) ? 'bg-red-50/40' : ''}`}>
                      ...
                      <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
                        ...
                        {task.due_date && (
                          <div className={`flex items-center gap-1 text-xs font-medium ${
                            ...
                            isDueToday(task.due_date) ? 'text-amber-600' : 'text-slate-400'
                          }`}>
                        ...
                        <button onClick={() => deleteTask(task.id)} className="text-slate-200 hover:text-red-400 transition-colors">
                          <X className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
