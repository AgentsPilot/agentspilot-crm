...
export default function BudgetPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([{ data: months }, { data: channels }]) => {
  ...
  const totals = budgetData.reduce(
    (acc, m) => ({
      allocated: acc.allocated + Number(m.allocated),
      spent: acc.spent + Number(m.spent),
      revenue: acc.revenue + Number(m.revenue),
    }),
    { allocated: 0, spent: 0, revenue: 0 }
  )
  const utilisation = totals.allocated > 0 ? ((totals.spent / totals.allocated) * 100).toFixed(1) : '0'
  const roi = totals.spent > 0 ? (((totals.revenue - totals.spent) / totals.spent) * 100).toFixed(0) : '0'
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {channelBudget.map(ch => {
                  const remaining = Number(ch.allocated) - Number(ch.spent)
                  const util = ch.allocated > 0 ? ((Number(ch.spent) / Number(ch.allocated)) * 100).toFixed(0) : '0'
                  return (
                    <tr key={ch.channel} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-700">${Number(ch.allocated).toLocaleString()}</td>
                      <td className="px-4 py-3 text-slate-700">${Number(ch.spent).toLocaleString()}</td>
              ...
              <tbody className="divide-y divide-gray-100">
                {budgetData.map(m => (
                  <tr key={m.month} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-700">${Number(m.allocated).toLocaleString()}</td>
                    <td className="px-4 py-3 text-slate-700">${Number(m.spent).toLocaleString()}</td>
                    <td className="px-4 py-3 text-emerald-600 font-medium">${Number(m.revenue).toLocaleString()}</td>
                    <td className="px-4 py-3"><span className="font-semibold text-amber-600">{m.roi}%</span></td>
                    <td className="px-4 py-3 text-emerald-600">${(Number(m.allocated) - Number(m.spent)).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
